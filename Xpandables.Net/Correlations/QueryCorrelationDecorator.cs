...
public sealed class QueryCorrelationDecorator<TQuery, TResult> : IQueryHandler<TQuery, TResult>
    where TQuery : class, IQuery<TResult>, ICorrelationDecorator
{
    ...
    public QueryCorrelationDecorator(CorrelationEvent correlationContext, IQueryHandler<TQuery, TResult> decoratee)
    {
    ...
    public async Task<IOperationResult<TResult>> HandleAsync(TQuery query, CancellationToken cancellationToken = default)
    {
        try
        {
            var resultState = await _decoratee.HandleAsync(query, cancellationToken).ConfigureAwait(false);
