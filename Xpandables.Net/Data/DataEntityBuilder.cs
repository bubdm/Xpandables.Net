...
namespace Xpandables.Net.Data
{
    ...
    public sealed class DataEntityBuilder
    {
        ...
        private static string KeyBuilder(Type type, bool isGeneric)
        {
        ...
        private static TDelegate GetTypeBuilderDelegate<TDelegate>()
            where TDelegate : Delegate
        {
        ...
        /// <param name="type">The type to act on.</param>
        private static TDelegate GetGenericBuilderDelegate<TDelegate>(Type type)
            where TDelegate : Delegate
        {
            var optionExpression = Expression.Parameter(typeof(DataOptions));
            var propertyExpression = Expression.Parameter(typeof(DataPropertyBuilder));
