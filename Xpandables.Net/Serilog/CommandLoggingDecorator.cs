...
namespace Xpandables.Net.Serilog
{
        ...
        where TCommand : class, ICommand, ILoggingDecorator
    {
        ...
        public CommandLoggingDecorator(ILoggerEngine logger, ICommandHandler<TCommand> decoratee)
        {
        ...
        public void Handle(TCommand command)
        {
            try
            {
                _logger.OnEntryLogAsync(_decoratee, command).RunSync();
                _decoratee.Handle(command);
            }
            catch (Exception exception)
            {
                _logger.OnExceptionLogAsync(_decoratee, command, exception).RunSync();
                throw;
            }
            finally
            {
                _logger.OnExitLogAsync(_decoratee, command).RunSync();
            }
        }
    }
}