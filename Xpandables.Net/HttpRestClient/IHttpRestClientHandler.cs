...
namespace Xpandables.Net.HttpRestClient
{
    ...
    /// You need to register the <see cref="IHttpRestClientEngine"/> with the appropriate extension method.
    ...
    public interface IHttpRestClientHandler : IDisposable
    {
        ...
        Task<HttpRestClientResponse<IAsyncEnumerable<TResult>>> HandleAsync<TResult>(IAsyncQuery<TResult> query, CancellationToken cancellationToken = default);
        ...
        /// <returns>Returns a task <see cref="HttpRestClientResponse{TResult}"/>.</returns>
        /// <exception cref="ArgumentNullException">The <paramref name="query"/> is null.</exception>
        Task<HttpRestClientResponse<IAsyncEnumerable<TResult>>> HandleAsync<TQuery, TResult>(TQuery query, CancellationToken cancellationToken = default)
