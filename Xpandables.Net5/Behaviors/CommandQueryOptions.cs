...
namespace System.Design.DependencyInjection
{
    ...
    public sealed class CommandQueryOptions
    {
        ...
        /// </summary>
        public CommandQueryOptions UseTransactionBehavior<TTransactionScopeProvider>()
            where TTransactionScopeProvider : class, ITransactionScopeProvider
            => this.With(cq => cq.IsTransactionEnabled = typeof(TTransactionScopeProvider));

        /// <summary>
        /// Enables identity data behavior to commands and queries that are decorated with the <see cref="IBehaviorIdentity"/>.
        /// </summary>
        /// <typeparam name="TIdentityProvider">The user type provider.</typeparam>
        public CommandQueryOptions UseIdentityBehavior<TIdentityProvider>()
            where TIdentityProvider : class, IIdentityProvider
            => this.With(cq => cq.IsIdentityDataEnabled = typeof(TIdentityProvider));

        internal bool IsValidatorEnabled { get; private set; }
        internal bool IsVisitorEnabled { get; private set; }
        internal Type? IsTransactionEnabled { get; private set; }
