...
namespace System
{
    ...
    /// You can make unconditional calls to its contents using <see cref="Linq"/> without testing whether the content is there or not.
    ...
    public readonly partial struct Optional<T> : IEnumerable<T>
    {
        ...
        public Optional<T> Map(Func<T, T> some) => IsValue() ? some(_values[0]) : this;
        ...
        /// <returns>An optional of <typeparamref name="T"/> type.</returns>
        /// <exception cref="ArgumentNullException">The <paramref name="some"/> is null.</exception>
        public async Task<Optional<T>> MapAsync(Func<T, Task<T>> some) => IsValue() ? await some(_values[0]).ConfigureAwait(false) : this;
        ...
        public Optional<T> WhenEmpty(Func<T> empty) => !IsValue() ? empty() : this;
        ...
        /// <returns>The replacement value.</returns>
        /// <exception cref="ArgumentNullException">The <paramref name="empty"/> is null.</exception>
        public async Task<Optional<T>> WhenEmptyAsync(Func<Task<T>> empty) => !IsValue() ? await empty().ConfigureAwait(false) : this;
