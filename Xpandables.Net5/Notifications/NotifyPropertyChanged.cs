...
namespace Xpandables.Net5.Notifications
{
    ...
    {
        /// <summary>
        /// Initializes a new instance of <see cref="NotifyPropertyChanged{T}"/> class and its <see cref="Dependencies"/>.
        /// </summary>
        protected NotifyPropertyChanged()
        {
            Dependencies = DependencyPropertiesProvider();
            PropertyChanged = (s, e) => { };
        }
        ...
        protected bool SetProperty<TValue, TProperty>(ref TValue storage, TValue value, Expression<Func<T, TProperty>> selector)
        ...
        protected bool SetProperty<TValue>(ref TValue storage, TValue value, Expression<Func<T, string>> selector)
            => SetProperty(ref storage, value, GetMemberNameFromExpression(selector));
        ...
        /// </summary>
        protected IDictionary<string, List<string>> DependencyPropertiesProvider()
        {
                            ...
                            && propertyNotifiers.Find(predicateFind) != null)
                        {
                            throw new InvalidOperationException("Circular dependency found.",
                                new ArgumentException(
                                    $"The {property.Name} owns a dependency on {dependency} which one depends on {property.Name}."));
                        }
