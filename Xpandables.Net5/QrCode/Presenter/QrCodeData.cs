...
namespace System.Design.QrCode
{
    ...
    public sealed class QrCodeData : Disposable
    {
        ...
        public QrCodeData(byte[] rawData, Compression compressionModel)
        {
            var bytes = new List<byte>(rawData);

            if (compressionModel == Compression.Deflate || compressionModel == Compression.GZip)
            {
                using var input = new MemoryStream(bytes.ToArray());
                using var output = new MemoryStream();
                using (var stream = compressionModel == Compression.Deflate
        ...
        public byte[] GetRawData(Compression compressMode)
        {
            ...
            if (compressMode == Compression.Deflate || compressMode == Compression.GZip)
            {
                using var output = new MemoryStream();
                using (var stream = compressMode == Compression.Deflate
