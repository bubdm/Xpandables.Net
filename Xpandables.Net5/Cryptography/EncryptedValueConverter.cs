...
namespace System.Design.Cryptography
{
    ...
    public sealed class EncryptedValueConverter : TypeConverter
    {
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
            => value switch
            {
                ...
                string source => source.Split(':').Length == 3
                    ? new EncryptedValue(source.Split(':')[0], source.Split(':')[1], source.Split(':')[2])
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
            => value?.GetType() == typeof(EncryptedValue) && destinationType == typeof(string)
                ? ((EncryptedValue)value).ToString("{0}:{1}:{2}", culture)
